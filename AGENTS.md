...
**Objetivo principal e modelo conceitual.** A biblioteca dá ao software que a utiliza duas coisas, estáveis entre reinícios: uma **identidade única** — *quem* é este agente: `AgentUUID` (UUIDv7 gerado e persistido), `AgentName` e `Hostname` — e uma **localização virtual** — *onde* ele está: `MachineID` (nó físico ou virtual) e `Workspace` (tenant lógico). `DataDir` é campo de suporte (onde as identidades geradas são persistidas), não identidade. Toda a maquinaria de persistência e concorrência existe para manter `AgentUUID` e `MachineID` iguais entre reinícios e entre processos irmãos.

A biblioteca resolve seis campos:
1. [`DataDir`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/identity.go#L228): Diretório persistente para armazenamento local de dados e arquivos de identidade (suporte).
...
- **Segurança Rigorosa:** Arquivos lidos dentro de `$DATADIR` não seguem symlinks: `Lstat` antes de abrir, `O_NOFOLLOW` no `open` (Linux, macOS e BSDs) e `os.SameFile` depois de abrir, impedindo exfiltração de segredos do host. Entradas como `.` e `..` são rejeitadas em `AGENT_NAME` e `WORKSPACE` para prevenir ataques de *path traversal*.
...
| [`resolve.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/resolve.go) | Motor de resolução pura: função [`resolve`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/resolve.go#L47) e struct [`resolver`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/resolve.go#L30). Cadeias de precedência de cada campo, pipeline de higienização de strings (`firstLine`, remoção de BOM UTF-8, trim de controles/NUL), leitura classificada dos arquivos auto-geridos (`readManaged`: ausente, válido ou corrompido, inclusive vazio), janela de estabilização contada do `mtime` (`readSettled`, `settleDeadline`), persistência durável (`persistGenerated`), detecção de corrupção e protocolo de convergência concorrente (`.regen`). |
...
| [`system_unix.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/system_unix.go) / [`system_windows.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/system_windows.go) | `openRegular`, a abertura de arquivo por plataforma: em Unix usa `O_NONBLOCK` (não trava em FIFO sem escritor) e, quando não deve seguir links, `O_NOFOLLOW`; em Windows delega a `os.Open`. |
| [`system_nofollow.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/system_nofollow.go) / [`system_nofollow_other.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/system_nofollow_other.go) | A constante `openNoFollow` e o classificador `isSymlinkRefusal` (`ELOOP`/`EMLINK`): reais em Linux, macOS e BSDs; neutros nos Unix cujo `syscall` não expõe `O_NOFOLLOW` (Solaris, illumos, AIX, DragonFly), onde a recusa de links fica só por conta do `Lstat` + `os.SameFile`. |
...
| [`system_unix_test.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/system_unix_test.go) | Garantias de `openRegular` que só existem em Unix: abrir um FIFO sem bloquear e recusar um link simbólico no próprio `open` (`O_NOFOLLOW`). |
| [`helpers_test.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/helpers_test.go) | Contrato individual de cada auxiliar (`preview`, `hasControlBytes`, `normalizeMachineID`, `isSpaceOrControl`, `isRelPathComponent`, classes de bytes, `validLabel`), fronteiras exatas da RFC 1123 e a prova de **zero alocações** dos validadores (`TestValidators_ZeroAllocs`). |
| [`validators_bench_test.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/validators_bench_test.go) | Benchmarks dos validadores e auxiliares REAIS do pacote (`go test . -bench=. -benchmem -run=XXX`), que sustentam as metas de `docs/07`. |
...
- **[`docs/`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/docs)**: **Especificações Técnicas Canônicas e Modulares do Projeto** (suficientes para refatorar a lib do zero com 100% de conformidade):
  ...
  - [`docs/06-ENGINEERING-LESSONS-AND-ANTI-REGRESSION.md`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/docs/06-ENGINEERING-LESSONS-AND-ANTI-REGRESSION.md): Catálogo de lições e prevenção de bugs (BUG-01 a BUG-24).
...
1. **Criação a Frio (Arquivo Inexistente):** Utiliza [`CreateExclusive`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/system.go#L158). Cria um arquivo temporário via `writeTemp` no mesmo diretório, executa `fchmod(0644)`, `fsync`, fecha e tenta vincular ao destino via `os.Link`. Se outro processo chegou primeiro (`ErrExist`), o perdedor aguarda por até 10 s (500 tentativas de 20 ms) e adota o conteúdo do arquivo existente através de `readSettled`, evitando falso descarte em volumes NFS. A janela é contada a partir do `mtime` do arquivo (`settleDeadline`): um arquivo vazio ou inválido modificado há mais de 10 s não é artefato de corrida e é declarado corrompido na hora (BUG-23). A leitura inicial dos arquivos auto-geridos (`readManaged`) distingue ausente de vazio e só manda o vazio para essa janela.
...
3. **Recuperação de Corrupção (Regeneração):** Se um arquivo existente em `$DATADIR` contiver dados inválidos (ex.: truncamento por crash, lixo ou arquivo vazio):
...
- **Diagnóstico Debug:** Se a env `LOGHUB_IDENT_DEBUG` estiver definida (qualquer valor não vazio), é impressa uma linha por campo resolvido em `stderr` (seis), precedida, quando houver, por linhas de diagnóstico sobre fontes ignoradas, tudo antes de qualquer falha. Exemplo:
  ```text
...
5. **Atualize Testes em Camadas Duplas:**
   - Se alterar regras de resolução ou novos campos:
     - Adicione testes unitários no [`resolve_test.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/resolve_test.go) com `fakeSystem`; um auxiliar novo ganha o seu próprio teste em [`helpers_test.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/helpers_test.go) (e entra em `TestValidators_ZeroAllocs` se for validador).
     - Adicione/atualize testes na suíte de integração em [`tests/api_test.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/tests/api_test.go) ou [`tests/exitcodes_test.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/tests/exitcodes_test.go). Mudou a API pública ou o `go.mod`? [`tests/examples_test.go`](file:///Users/patrickbrandao/Projects/loghub/go-loghub-ident/tests/examples_test.go) compila e roda os exemplos de `skill/examples/`.
